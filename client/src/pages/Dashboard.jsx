...
export default function Dashboard({ user }) {
  ...
  const logPrayer = (prayerName) => {
    ...
    // هنا نضيف API call لتسجيل الصلاة
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        {prayers.map(prayer => (
          <div 
            key={prayer.name}
            className={`prayer-card text-center p-8 rounded-3xl shadow-lg border-4 transition-all duration-300 cursor-pointer ${
          ...
          >
            ...
            <button className="w-full bg-white/20 backdrop-blur-sm text-white font-semibold py-3 px-6 rounded-2xl hover:bg-white/30 transition-all">
